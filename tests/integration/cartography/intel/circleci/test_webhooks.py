...
import tests.data.circleci.webhooks
...
@patch.object(
    cartography.intel.circleci.webhooks,
    "get",
    return_value=tests.data.circleci.webhooks.CIRCLECI_WEBHOOKS,
)
def test_load_circleci_webhooks(mock_api, neo4j_session):
    # Arrange
    ...
    assert check_nodes(
        neo4j_session, "CircleCIWebhook", ["id", "has_signing_secret"]
    ) == {
        ("wh-1", True),
    }

    # Assert (Project)-[:RESOURCE]->(Webhook)
    assert check_rels(
        neo4j_session,
        "CircleCIWebhook",
        "id",
