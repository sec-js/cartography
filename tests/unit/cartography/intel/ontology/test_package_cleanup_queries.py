...
def test_package_version_cleanup_queries_cover_derived_relationships():
    queries = build_cleanup_queries(PackageVersionSchema())
    ...
    expected_rel_clauses = [
        ...
        "MATCH (n)-[r:DEPENDS_ON]->(:PackageVersion)",
    ...
    for clause in expected_rel_clauses:
        ...
        ), f"Missing cleanup query for package version relationship clause: {clause}"


def test_package_cleanup_queries_cover_has_version():
    queries = build_cleanup_queries(PackageSchema())

    clause = "MATCH (n)-[r:HAS_VERSION]->(:PackageVersion)"
    assert any(
        clause in query for query in queries
