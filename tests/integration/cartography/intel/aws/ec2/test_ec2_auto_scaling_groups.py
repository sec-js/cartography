...
@patch.object(
    ...
    return_value=GET_AUTO_SCALING_GROUPS,
)
@patch.object(
    auto_scaling_groups,
    "get_launch_configurations",
    return_value=GET_LAUNCH_CONFIGURATIONS,
)
@patch.object(
    instances,
    "get_ec2_instances",
...
def test_sync_ec2_auto_scaling_groups(
    mock_get_instances,
    mock_get_launch_configs,
    mock_get_asgs,
    neo4j_session,
):
    """
    ...
    assert check_nodes(neo4j_session, "AWSAutoScalingGroup", ["arn", "name"]) == {
        ...
        (
            ...
            GET_AUTO_SCALING_GROUPS[1]["AutoScalingGroupName"],
        ),
    }
