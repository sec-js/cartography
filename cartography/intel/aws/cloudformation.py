...
# Stack states where UpdateStack is valid. Stacks in terminal or failed states
# (DELETE_COMPLETE, ROLLBACK_COMPLETE, DELETE_FAILED, UPDATE_ROLLBACK_FAILED, etc.)
# reject UpdateStack with a ValidationError, so ingesting them would create
# false-positive CAN_EXEC escalation paths.
UPDATABLE_STACK_STATES = {
    "CREATE_COMPLETE",
    "UPDATE_COMPLETE",
...
def get_cloudformation_stacks(
    ...
    for page in paginator.paginate():
        stacks.extend(
            ...
            if s.get("StackStatus") in UPDATABLE_STACK_STATES
        )
    return stacks
...
def sync(
    ...
    merge_module_sync_metadata(
        ...
        synced_type="AWSCloudFormationStack",
