...
class NSGToTagRelProperties(CartographyRelProperties):
    lastupdated: PropertyRef = PropertyRef("lastupdated", set_in_kwargs=True)


@dataclass(frozen=True)
class NSGToTagRel(CartographyRelSchema):
    """An Azure network security group has the tag."""
    ...
    properties: NSGToTagRelProperties = NSGToTagRelProperties()


@dataclass(frozen=True)
class AzureNetworkSecurityGroupTagsSchema(CartographyNodeSchema):
