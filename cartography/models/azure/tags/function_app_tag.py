...
class FunctionAppToTagRelProperties(CartographyRelProperties):
    lastupdated: PropertyRef = PropertyRef("lastupdated", set_in_kwargs=True)


@dataclass(frozen=True)
class FunctionAppToTagRel(CartographyRelSchema):
    """An Azure Function App has the tag."""

    target_node_label: str = "AzureFunctionApp"
    target_node_matcher: TargetNodeMatcher = make_target_node_matcher(
        {"id": PropertyRef("resource_id")},
    )
    direction: LinkDirection = LinkDirection.INWARD
    rel_label: str = "TAGGED"
    properties: FunctionAppToTagRelProperties = FunctionAppToTagRelProperties()


@dataclass(frozen=True)
class AzureFunctionAppTagsSchema(CartographyNodeSchema):
