...
from cartography.intel.modal.util import list_dicts
from cartography.intel.modal.util import ModalClient
from cartography.models.modal.dict import ModalDictSchema
...
async def sync(
    ...
    """Ingest the Dicts of one environment. Contents are not enumerated."""
    environment_name = common_job_parameters["ENVIRONMENT_NAME"]
    raw = await list_dicts(client, environment_name)
...
def cleanup(
    ...
    GraphJob.from_node_schema(ModalDictSchema(), common_job_parameters).run(
        neo4j_session
    )
