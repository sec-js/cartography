...
class AppServiceToTagRelProperties(CartographyRelProperties):
    lastupdated: PropertyRef = PropertyRef("lastupdated", set_in_kwargs=True)


@dataclass(frozen=True)
class AppServiceToTagRel(CartographyRelSchema):
    """An Azure App Service has the tag."""

    target_node_label: str = "AzureAppService"
    target_node_matcher: TargetNodeMatcher = make_target_node_matcher(
        {"id": PropertyRef("resource_id")},
    )
    direction: LinkDirection = LinkDirection.INWARD
    rel_label: str = "TAGGED"
    properties: AppServiceToTagRelProperties = AppServiceToTagRelProperties()
