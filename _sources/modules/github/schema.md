...
graph LR
    ...
    GitHubOrganization -- RESOURCE --> GitHubExternalIdentity
    ...
    GitHubUser -- HAS_IDENTITY --> GitHubExternalIdentity
...
| Field | Index | Description |
|-------|-------|-------------|
| id | Yes | Organization URL and GitHub external identity node ID. |
| firstseen |  | Timestamp when a sync job first created this node. |
| lastupdated | Yes | Timestamp of the last sync that observed this node. |
| saml_name_id |  | SAML NameID supplied by the organization's identity provider; not necessarily an email address. |
| saml_name_id_normalized |  | Trimmed, lowercase SAML NameID for canonical email matching; the original value remains in saml_name_id. |

#### Relationships

- `(:GitHubUser)-[:HAS_IDENTITY]->(:GitHubExternalIdentity)`: Links a GitHub account to its organization-specific external identity.

- `(:GitHubOrganization)-[:RESOURCE]->(:GitHubExternalIdentity)`: Scopes an external identity to the organization that federates it.
...
| normalized_emails |  | Distinct nonempty public and organization verified-domain emails, normalized with the canonical user email policy for SAML conflict checks. |
