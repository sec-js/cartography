...
AWS = ScopeById("AWSAccount", "AWS_ID")
AZURE = ScopeById("AzureSubscription", "AZURE_SUBSCRIPTION_ID")
ENTRA = ScopeById("EntraTenant", "TENANT_ID")
GCP = ScopeById("GCPProject", "PROJECT_ID")
K8S = ScopeById("KubernetesCluster", "CLUSTER_ID")
SEMGREP = ScopeById("SemgrepDeployment", "DEPLOYMENT_ID")
...
CLEANUP_CASES = [
    ...
    pytest.param(
        RelationshipEffect("PackageVersion", "DEPLOYED", "Image"),
        None,
        _rel_cleanup("PackageVersion", "DEPLOYED", "Image"),
        id="ontology_packages_deployed",
    ),
    pytest.param(
        RelationshipEffect("TrivyImageFinding", "AFFECTS", "PackageVersion"),
        None,
        _rel_cleanup("TrivyImageFinding", "AFFECTS", "PackageVersion"),
        id="ontology_packages_affects",
    ),
    pytest.param(
        RelationshipEffect("PackageVersion", "SHOULD_UPDATE_TO", "TrivyFix"),
        None,
        _rel_cleanup("PackageVersion", "SHOULD_UPDATE_TO", "TrivyFix"),
        id="ontology_packages_should_update_to",
    ),
    pytest.param(
        RelationshipEffect("PackageVersion", "DEPENDS_ON", "PackageVersion"),
        None,
        _rel_cleanup("PackageVersion", "DEPENDS_ON", "PackageVersion"),
    ...
    pytest.param(
        PropertyEffect(
            "ScalewayInstance", ("exposed_internet", "exposed_internet_type")
        ),
        None,
