...
def test_sync_cosmosdb_accounts(
    ...
    # Assert - Account fields come out of the SDK `properties` block, not flat. These
    # would all be null if the payload were not normalized.
    assert check_nodes(
        neo4j_session,
        "AzureCosmosDBAccount",
        [
        ...
        (
            da1,
            "https://ddb1.documents.azure.com:443/",
            "Succeeded",
            "Standard",
            True,
            True,
            ...
            "Enabled",
            "Small",
            "https://kv1.vault.azure.net/keys/cmk",
            "Session",
            100,
            5,
        ),
        (
            da2,
            "https://ddb1.documents.azure.com:444/",
            "Succeeded",
            "Standard",
            True,
            True,
    ...
    # Assert - list-valued account properties (check_nodes cannot hash lists)
    assert (
        neo4j_session.run(
            """
        MATCH (a:AzureCosmosDBAccount{id: $id})
        RETURN a.capabilities AS capabilities, a.ipranges AS ipranges
        """,
            id=da1,
        ).single()
        == {"capabilities": ["EnableMongo"], "ipranges": ["10.0.0.0/24"]}
    )

    # Assert - Regional locations are ingested. These vanish entirely (and get cleaned up
    # on the next sync) when the nested account collections are not normalized.
    assert check_nodes(
        neo4j_session,
        "AzureCosmosDBLocation",
        ["id", "locationname", "failoverpriority"],
    ) == {
        ("DA1-eastus", "East US", 0),
        ("DA1-centralindia", "Central India", 0),
        ("DA1-japaneast", "Japan East", 0),
    }
    assert check_rels(
        neo4j_session,
        "AzureCosmosDBAccount",
    ...
    assert check_nodes(
        ...
        ("DA1-eastus", "East US", 0),
        ("DA2-eastus", "East US", 0),
    }

    # Assert - CORS policies. The unique id is generated by cartography, so assert on the
    # normalized policy fields instead.
    assert check_nodes(
        neo4j_session,
        "AzureCosmosDBCorsPolicy",
        ["allowedorigins", "allowedmethods", "maxageinseconds"],
    ) == {
        ("*", "GET,POST", 3600),
        ("*", "GET", 600),
    }

    # Assert - Virtual network rules
    assert check_nodes(
        neo4j_session,
        "AzureCosmosDBVirtualNetworkRule",
        ["id", "ignoremissingvnetserviceendpoint"],
    ) == {
        (rg + "/providers/Microsoft.Network/virtualNetworks/vn1", False),
        (rg + "/providers/Microsoft.Network/virtualNetworks/vn2", False),
    }

    # Assert - Private endpoint connections. Losing these silently stops modelling the
    # network exposure of a Cosmos account.
    assert check_nodes(
        neo4j_session,
        "AzureCDBPrivateEndpointConnection",
        ["id", "privateendpointid", "status", "actionrequired"],
    ) == {
        (
            da1 + "/privateEndpointConnections/pe1",
            rg + "/providers/Microsoft.Network/privateEndpoints/pe1",
            "Approved",
            "None",
        ),
        (
            da2 + "/privateEndpointConnections/pe2",
            rg + "/providers/Microsoft.Network/privateEndpoints/pe2",
            "Approved",
            "None",
        ),
    }

    # Assert - Child resources read throughput out of `properties.options` and the
    # container fields out of `properties.resource`.
    assert check_nodes(
        neo4j_session,
        "AzureCosmosDBSqlDatabase",
        ["id", "throughput", "maxthroughput"],
    ) == {
        (da1 + "/sqlDatabases/sql_db1", 100, 1000),
        (da2 + "/sqlDatabases/sql_db2", 100, 1000),
    }
    assert check_nodes(
        neo4j_session,
        "AzureCosmosDBSqlContainer",
    ...
    assert check_nodes(
        neo4j_session,
        "AzureCosmosDBMongoDBCollection",
        ["id", "throughput", "maxthroughput", "collectionname", "analyticalttl"],
    ) == {
        (
            da1 + "/mongodbDatabases/mongo_db1/mongodbCollections/col1",
            100,
