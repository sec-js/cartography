...
graph LR
    ...
    SnowflakeAccount -- RESOURCE --> SnowflakeInheritedGrant
...
| Field | Index | Description |
|-------|-------|-------------|
| id | Yes | Account-scoped identifier derived from the grantee, container, object type, privilege, and grantor. |
| firstseen |  | Timestamp when a sync job first created this node. |
| lastupdated | Yes | Timestamp of the last sync that observed this node. |
| container_type |  | ACCOUNT, DATABASE, or SCHEMA scope. |
| created_on |  | Timestamp when Snowflake created the grant. |
| database_name |  | Source database for DATABASE and SCHEMA scope. |
| grant_option |  | Whether the recipient can grant this privilege to others. |
| granted_by |  | Role that authorized the grant; absent for system grants. |
| grantee_name |  | Recipient name as reported by Snowflake. |
| grantee_type |  | Provider grantee kind; unsupported principal kinds retain their identity here. |
| object_type |  | Snowflake object type covered by this grant, such as TABLE or VIEW. |
| privilege |  | Privilege on matching objects inside the container, not on the container itself. |
| schema_name |  | Source schema for SCHEMA scope. |

#### Relationships

- `(:SnowflakeInheritedGrant)-[:APPLIES_IN]->(:SnowflakeSecurable)`: The inherited grant applies inside this account, database, or schema.

- `(:SnowflakePrincipal)-[:HAS_INHERITED_GRANT]->(:SnowflakeInheritedGrant)`: A principal holds a grant applying to matching current and future objects.
