...
def test_sync_redshift_clusters(mock_get_data, neo4j_session):
    """
    Ensure that sync() creates AWSRedshiftCluster nodes and all expected relationships.
    """
    ...
    # Verify AWSRedshiftCluster node
    assert check_nodes(neo4j_session, "AWSRedshiftCluster", ["id"]) == {
        ("arn:aws:redshift:us-east-1:1111:cluster:my-cluster",),
    }

    # Verify AWSAccount -[:RESOURCE]-> AWSRedshiftCluster
    assert check_rels(
        neo4j_session,
        "AWSAccount",
        "id",
    ...
    # Verify AWSRedshiftCluster -[:STS_ASSUMEROLE_ALLOW]-> AWSPrincipal
    assert check_rels(
        neo4j_session,
        "AWSRedshiftCluster",
        "id",
    ...
    # Verify AWSRedshiftCluster -[:MEMBER_OF_AWS_VPC]-> AWSVpc
    assert check_rels(
        neo4j_session,
        "AWSRedshiftCluster",
        "id",
