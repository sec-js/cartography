...
| error_code |  | AWS error code returned by the API call, such as `AccessDenied`. Null when the call succeeded; a non-null value means CloudTrail recorded an error for the call. |
| error_message |  | AWS error message returned by the API call. Null when the call succeeded. |
...
| user_type |  | Type of Spacelift user: `human` when the run's `triggeredBy` field is an email address, otherwise `system`. |
