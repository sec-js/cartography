...
| firstseen |  | Timestamp when a sync job first created this node. |
| lastupdated | Yes | Timestamp of the last sync that observed this node. |
| all_applications |  | True if this is the special 'All Applications' group, which implicitly contains every non-managed application of the organization. |
| all_users |  | True if this is the special 'All Users' group, which implicitly contains every user of the organization. |
...
| has_object_storage_private_access |  | True if the Private Network is enabled for Object Storage private access. |
...
| status |  | Project status (`active`, `deleting`). |
| updated_at |  | Last update timestamp |
| *_ont_name* | Yes | Normalized field sourced from `name`. |
| *_ont_source* |  | Module that populated this node's ontology fields. |
...
| exposed_internet | Yes | `True` when `is_public` is true, meaning a publicly reachable endpoint is provisioned. |
| exposed_internet_type | Yes | How it is exposed. Always `direct`. |
| high_availability_mode |  | High-availability topology of the instance (`disabled`, `single_zone`, `multiple_zone`). Refines `is_ha_cluster`, which only says whether HA is on. |
...
| name |  | VPC name. |
| object_storage_private_access_enabled |  | True if Object Storage private access is enabled for the VPC. |
...
| transitivity_enabled |  | True if the VPC allows packets from peered VPCs to transit through it. |
